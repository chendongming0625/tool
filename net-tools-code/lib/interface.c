...
#include "intl.h"
#if 0
#include "../cel/include/arraylist.h"
#else
#include "arraylist.h"
#endif
#ifdef IFF_PORTSEL
...
void free_interface_info(void *p)
{
	if (p)
	{
		free(p);
...
CelArrayList *for_all_get_interfaces(InterFaceInfo*(*doit)(struct interface *, void *),void *cookie)
{
	struct interface *ife;
	CelArrayList *array_list = cel_arraylist_new(free_interface_info);
	if (!if_list_all && (if_readlist() < 0))
		return NULL;

	for (ife = int_list; ife; ife = ife->next) {
		InterFaceInfo*info = doit(ife, cookie);
		if (info)
		{
			cel_arraylist_push_back(array_list, info);
		}
...
InterFaceInfo *do_if_get(struct interface *ife, void *cookie)
{
	...
	if (res >= 0) 
	{
		...
		memcpy(info->name, ife->name, sizeof(info->name));

		if (ife->has_ip)//»ñÈ¡ip
		{
			const struct aftype *ap = get_afntype(ife->addr.sa_family);
			if (ap == NULL)
				ap = get_afntype(0);
			memcpy(info->ip, ap->sprint(&ife->addr_sas, 1),
				sizeof(info->ip));
		}
		else
		...
		{
			memcpy(info->mac, hw->print(ife->hwaddr), sizeof(info->mac));
			if (ife->flags & IFF_UP)
				memcpy(info->status, "UP", sizeof("UP"));
			else
				memcpy(info->status, "DOWN", sizeof("DOWN"));
...
void ife_print_long(struct interface *ptr)
{
	...
	printf(_("        %s %s"), ap->name,
	       ap->sprint(&ptr->addr_sas, 1));//Êä³öip
