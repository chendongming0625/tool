...
int rprint_fib(int ext, int numeric)
{
    ...
    FILE *fp = fopen(_PATH_PROCNET_ROUTE, "r");//获取路由表其实就是读取系统目录下的指定文件
    char *fmt;
...
int rget_fib(int ext, int numeric, CelArrayList*routes_list)
{
	...
	while (fgets(buff, 1023, fp)) {
		...
		{
#if HAVE_RTF_REJECT
			struct _RouteInfo *route_info = (struct _RouteInfo*)malloc(sizeof(struct _RouteInfo));
			memset(route_info,0,sizeof(struct _RouteInfo));
			get_uuid(route_info->id);
			memcpy(route_info->dst, net_addr, sizeof(route_info->dst));
			memcpy(route_info->gateway_addr, gate_addr, sizeof(route_info->gateway_addr));
			memcpy(route_info->netmask, mask_addr, sizeof(route_info->netmask));
			memcpy(route_info->flags, flags, sizeof(route_info->flags));
			route_info->metric = metric;
			memcpy(route_info->ifname, iface, sizeof(route_info->ifname));
			cel_arraylist_push_back(routes_list, route_info);			
#endif
		}
	}
	free(fmt);
