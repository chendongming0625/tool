...
CelArrayList * get_interface_info(const char * interface_name)
{
	...
	if (!interface_name) 
	{
		CelArrayList *interface_list = for_all_get_interfaces(do_if_get, &opt_a);
		close(skfd);
	...
	{
		struct interface *ife;
		ife = lookup_interface(interface_name);
		if (!ife) {
			close(skfd);
			return NULL;
		}
		CelArrayList *interface_list = cel_arraylist_new(free_interface_info);
		InterFaceInfo *info = do_if_get(ife, &opt_a);
		cel_arraylist_push_back(interface_list, info);
		close(skfd);
		return interface_list;
	}
}
int edit_if(char *if_name,char *ip_addr, char *status)
{
	...
	safe_strncpy(ifr.ifr_name, if_name, IFNAMSIZ);//获取网卡名
	char host[128] = { 0 };
	const struct aftype *ap = get_aftype(DFLT_AF);
	if (ap)
	{
		addr_family = ap->af;
		skfd = ap->fd;
	...
	if (ip_addr)
	{
		safe_strncpy(host, ip_addr, (sizeof host));//获取ip地址
		if (ap->input == NULL)
		{
			close(skfd);
			return -1;
		}
		if (ap->input(0, host, &_sa) < 0) //转换成网络地址到_sa中
		{
			if (ap->herror)
				ap->herror(host);
			else
			{
				printf("ifconfig: error resolving\n");
				close(skfd);
				return -1;
			}		
		}
		...
		{
			int fd = get_socket_for_af(AF_INET);
			if (fd < 0)
			{
				printf("No support for INET on this system.\n");
				close(skfd);
				return -1;
			}
	...
	if (status)
	{
		...
		{
			set_flag(ifr.ifr_name, (IFF_UP | IFF_RUNNING));	//启动网卡
		}
		else if (!strcasecmp(status, "down"))
		{
			clr_flag(ifr.ifr_name, IFF_UP);//down 掉网卡
		}
	}
	close(skfd);
	return 0;
...
//    spp = argv;
//    safe_strncpy(ifr.ifr_name, *spp++, IFNAMSIZ);//获取网卡名
//    if (*spp == (char *) NULL) {
...
//	safe_strncpy(host, *spp, (sizeof host));//获取ip地址
//	/* FIXME: sa is too small for INET6 addresses, inet6 should use that too,
//	   broadcast is unexpected */
//	if (ap->getmask) {
//		//INET_getnetmask() 查找是否还存在掩码 不存在返回0
